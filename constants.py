...
SUPPORTED_EXTENSIONS = {
    ...
    ".docx": Docx2txtLoader,
    ".csv": CSVLoader,
    ".txt": lambda p: TextLoader(p, autodetect_encoding=True),  # ←変更
}
...
# RAG パラメータ追記
RETRIEVER_K = 5       # 何件取り出すか
CHUNK_SIZE = 500      # チャンクサイズ
CHUNK_OVERLAP = 50    # チャンク重なり

# 追記
def format_file_info(path: str, page_number: int | None = None) -> str:
    """ファイルパスに（ページ番号）を付けて表示用に整える"""
    if page_number is not None:
        return f"{path}（{page_number}ページ目）"
    return str(path)